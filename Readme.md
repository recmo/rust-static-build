...
docker run --rm \
    -v "$(pwd)":/src \
    ghcr.io/recmo/rust-static-build:1.63-x86_64 \
    cargo build --release
docker run --rm \
    -v "$(pwd)":/src \
    ghcr.io/recmo/rust-static-build:1.63-aarch64 \
    cargo build --release
```
...
for host in amd64 arm64; do
    for target in x86_64 aarch64; do
        docker build --platform linux/$host --build-arg TARGET=$target --tag ghcr.io/recmo/rust-static-build:1.63-$target-$host .
        docker push ghcr.io/recmo/rust-static-build:1.63-$target-$host
    done
done
```
...
```
for target in x86_64 aarch64; do
    docker manifest rm ghcr.io/recmo/rust-static-build:1.63-$target
    docker manifest create \
        ghcr.io/recmo/rust-static-build:1.63-$target \
        ghcr.io/recmo/rust-static-build:1.63-$target-amd64 \
        ghcr.io/recmo/rust-static-build:1.63-$target-arm64
    docker manifest inspect ghcr.io/recmo/rust-static-build:1.63-$target
    docker manifest push ghcr.io/recmo/rust-static-build:1.63-$target
done
```
...
for host in amd64 arm64; do
    for target in x86_64 aarch64; do
        docker run --pull always --platform linux/$host --rm -it ghcr.io/recmo/rust-static-build:1.63-$target cargo --version
    done
done
```
